...
public class SpendGoldUseCaseTests
{
    ...
    private SpendGoldUseCase CreateUseCase()
        => new(_repositoryMock.Object, _transactionScopeMock.Object);

    [Fact]
    public async Task ExecuteAsync_Should_Throw_When_User_Not_Found()
    {
        var command = new SpendGoldCommand(1, 100);

        _transactionScopeMock
            .Setup(x => x.ExecuteAsync(It.IsAny<Func<CancellationToken, Task<SpendGoldResult>>>(), It.IsAny<CancellationToken>()))
            .Returns<Func<CancellationToken, Task<SpendGoldResult>>, CancellationToken>((func, ct) => func(ct));
    ...
    [Fact]
    public async Task ExecuteAsync_Should_Spend_Gold_When_Sufficient()
    {
        var command = new SpendGoldCommand(1, 50);

        var currency = CurrencyData.Create(1);
        currency.SetGold(100);
    ...
    [Fact]
    public async Task ExecuteAsync_Should_Throw_When_Insufficient_Gold()
    {
        var command = new SpendGoldCommand(1, 200);
